...
namespace HAENGSUNG_HNSMES_UI.Class
{
    class SharedAPI
    {
        ...
        public int ConnectRemoteServer(string server)
        {
            ...
            ns.lpProvider = null;
            int result = 0;
            if (server == @"\\10.3.20.18\CRIMP" || server == @"\\10.3.10.18\CRIMP")
            {
                result = WNetUseConnection(IntPtr.Zero, ref ns, "ethmes123!@#", "crimping", flags,
                                            sb, ref capacity, out resultFlags);
            }
            else
            {
                result = WNetUseConnection(IntPtr.Zero, ref ns, "", "", flags,
                                                sb, ref capacity, out resultFlags);
            }
            return result;
        }

...
{
    public partial class xucSMTLineType2 : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void StopMonitoring()
        {
            ...
            if (bgwSMT3.IsBusy) bgwSMT3.CancelAsync();
            if (bgwSMT4.IsBusy) bgwSMT4.CancelAsync();
            ...
            if (result.ResultInt == 0)
            {
                foreach (DataRow dr in result.ResultDataSet.Tables[0].Rows)
                {
                    ...
                    else if (dr["UNITNO"] + "" == "2")
                    {
                        ...
                        if (dr["LOCKFLAG"] + "" == "Y" || dr["ISSTKERR"] + "" == "Y")
                        {
                            if (dr["LOCKFLAG"] + "" == "Y") rsSMT2.FillColor = Color.Red;
                            if (dr["ISSTKERR"] + "" == "Y") rsSMT2.FillColor = Color.DarkOrange;
                        ...
                        {
                            if (bgwSMT2.IsBusy) bgwSMT2.CancelAsync();
                            rsSMT2.Location = new Point(rsSMT2.Location.X, this.Height);
                            if (picSMT2.InvokeRequired) picSMT2.Invoke(new PictureEditEventHandler(PictureEditEvent), picSMT2);
                        }
                    }
                    else if (dr["UNITNO"] + "" == "3")
                    {
                        if (txtMissRate3.InvokeRequired) txtMissRate3.Invoke(new TextEditEventHandler(TextEditEvent), txtMissRate3, dr["MISSRATE"] + "%/" + dr["PICKUPRATE"] + "%");

                        if (dr["LOCKFLAG"] + "" == "Y" || dr["ISSTKERR"] + "" == "Y")
                        {
                            if (dr["LOCKFLAG"] + "" == "Y") rsSMT3.FillColor = Color.Red;
                            if (dr["ISSTKERR"] + "" == "Y") rsSMT3.FillColor = Color.DarkOrange;

                            if (!bgwSMT3.IsBusy) bgwSMT3.RunWorkerAsync();
                        }
                        else
                        {
                            if (bgwSMT3.IsBusy) bgwSMT3.CancelAsync();
                            rsSMT3.Location = new Point(rsSMT3.Location.X, this.Height);
                            if (picSMT3.InvokeRequired) picSMT3.Invoke(new PictureEditEventHandler(PictureEditEvent), picSMT3);
                        }
                    }
                    else if (dr["UNITNO"] + "" == "4")
                    {
                        if (txtMissRate4.InvokeRequired) txtMissRate4.Invoke(new TextEditEventHandler(TextEditEvent), txtMissRate4, dr["MISSRATE"] + "%/" + dr["PICKUPRATE"] + "%");

                        if (dr["LOCKFLAG"] + "" == "Y" || dr["ISSTKERR"] + "" == "Y")
                        {
                            if (dr["LOCKFLAG"] + "" == "Y") rsSMT4.FillColor = Color.Red;
                            if (dr["ISSTKERR"] + "" == "Y") rsSMT4.FillColor = Color.DarkOrange;

                            if (!bgwSMT4.IsBusy) bgwSMT4.RunWorkerAsync();
                        }
                        else
                        {
                            if (bgwSMT4.IsBusy) bgwSMT4.CancelAsync();
                            rsSMT4.Location = new Point(rsSMT4.Location.X, this.Height);
                            if (picSMT4.InvokeRequired) picSMT4.Invoke(new PictureEditEventHandler(PictureEditEvent), picSMT4);
                        }
                    }
                }
            }
        ...
        private void bgwSMT3_DoWork(object sender, DoWorkEventArgs e)
        {
            bool bReverse = true;

            while (!bgwSMT3.CancellationPending)
            {
                try
                {
                    if (this.InvokeRequired) this.Invoke(new RectangleShapeEventHandler(RectangleShapeEvent), rsSMT3, picSMT3, picSMT3.Location, picSMT3.Size, bReverse);
                }
                catch { }
        ...
        private void bgwSMT4_DoWork(object sender, DoWorkEventArgs e)
        {
            bool bReverse = true;

            while (!bgwSMT4.CancellationPending)
            {
                try
                {
                    if (this.InvokeRequired) this.Invoke(new RectangleShapeEventHandler(RectangleShapeEvent), rsSMT4, picSMT4, picSMT4.Location, picSMT4.Size, bReverse);
                }
                catch { }

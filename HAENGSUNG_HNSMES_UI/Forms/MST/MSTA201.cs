...
namespace HAENGSUNG_HNSMES_UI.Forms.MST
{
    ...
    ///==========================================================
    ///</summary>
    public partial class MSTA201 : HAENGSUNG_HNSMES_UI.Forms.BASE.Form, Class.itfButton
    {
        // ******************************************* Base From Class ************************************
        ...
        private void MSTA201_Load(object sender, EventArgs e)
        {


        }

        private void MSTA201_Shown(object sender, EventArgs e)
        {
        ...
        public void SaveButton_Click()
        {
            ...
            string _strPlantCode = "";
            string _strPlantName = "";
            string _strUseFlag = "";
            string _strRemarks = "";
                ...
                foreach (DataRow _dr in _dt.Rows)
                {
                    switch (_dr.RowState)
                    {
                        case DataRowState.Modified:
                            _strPlantCode = _dr["PLANT"].ObjectNullString();
                            _strPlantName = _dr["PLANTNAME"].ObjectNullString();
                            _strUseFlag = _dr["USEFLAG"].ObjectNullString();
                            _strRemarks = _dr["REMARKS"].ObjectNullString();

                            if (!this.SaveData(_strPlantCode, _strPlantName, _strUseFlag, _strRemarks, "N"))
                            {
                                MainButton_INIT.PerformClick();
                                gvList.EX_GetFocuseRowCell("PLANT", _strPlantCode);
                                return;
                            }
            ...
            else if (base.CurrentDataTYPE == IDAT.Devexpress.FORM.UPDATEITEMTYPE.New)
            {
                ...
                _strPlantCode = txtPlant.Text.Trim().ObjectNullString();
                _strPlantName = txtPlantName.Text.Trim().ObjectNullString();
                _strUseFlag = rdgUseFlag.EditValue.ObjectNullString();
                _strRemarks = memRemarks.Text.Trim().ObjectNullString();

                if (!this.SaveData(_strPlantCode, _strPlantName, _strUseFlag, _strRemarks, "Y"))
                {
                    MainButton_INIT.PerformClick();
                    return;
                }
        ...
        private void GetGridViewList()
        {
            BASE_DXGridHelper.Bind_Grid( gcList
                                       ...
                                       , "1" }
                                       , true
                                       , "PLANT, PLANTNAME, USEFLAG, REMARKS");

            gvList.BestFitColumns();

        }

        private bool SaveData(string p_strPlantCode, string p_strPlantName, string p_strUseFlag, string p_strRemarks, string p_strNewflag)
        {
            bool _blReturn = BASE_db.Execute_Proc( "PKGBAS_BASE.PUT_PLANT"
                                                 , 1
                                                 , new string[] {
                                                   "A_CLINET"
                                                 , "A_COMPANY"
        ...
        private void gvList_RowStyle(object sender, RowStyleEventArgs e)
        {
            ...
            foreach (DataRow dr in changes.Rows)
            {
                if (dr["PLANT"] != null)
                {
                    ...
                    if (dr["PLANT"].ToString() == gvList.GetDataRow(e.RowHandle)["PLANT"].ToString())
                    {

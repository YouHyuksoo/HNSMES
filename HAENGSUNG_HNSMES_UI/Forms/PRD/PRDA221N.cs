...
namespace HAENGSUNG_HNSMES_UI.Forms.PRD
{
    ...
    ///==========================================================
    ///</summary>
    public partial class PRDA221N : HAENGSUNG_HNSMES_UI.Forms.BASE.Form, HAENGSUNG_HNSMES_UI.Class.itfButton, HAENGSUNG_HNSMES_UI.Class.itfScanner
    {
        ...
        private void PRDA221N_Load(object sender, EventArgs e)
        {

        }

        private void PRDA221N_Shown(object sender, EventArgs e)
        {
            InitForm();
        }
        ...
        private void ProcessScanEvent(string sType, string sData)
        {
            ...
            switch (sType)
            {
                ...
                case "ENTER":
                    if (btnInspection.Appearance.BackColor == Color.LawnGreen)
                    {
                        if(lblStatus.Text == "Inspecting..." || lblStatus.Text.EndsWith("NG"))
                            Set_CircuitInspection("FAIL");
                    }
                    break;
                ...
                case "NG":
                    if (btnInspection.Appearance.BackColor == Color.LawnGreen)
                    {
                        ...
                        //if (txtSerial.EditValue.ObjectNullString() == "")
                        if (txtWorkOrder.EditValue.ObjectNullString() == string.Empty)
                        {
                            iDATMessageBox.ErrorMessage("MSG_ER_PRD_128", this.Text, 3); //제품 바코드를 스캔하세요.
                        }
                        else
                        {
                            if (sType == "NG")
                            ...
                            {
                                // 양품처리
                                if (sType == "OK" && lblStatus.Text != "Inspecting...") return;
                                Set_CircuitInspection(sData);
                            }
                        }
                    }
                    break;
        ...
        private void InitForm()
        {
            ...
            /*JIG(지그) 정보*/
            BASE_DXGridLookUpHelper.Bind_GridLookUpEdit(glePin
        ...
        private void SetButton(object sender, EventArgs e)
        {
            ...
            if (btn.Name != "btnProdReady")
            {
                if (Global.Global_Variable.EHRCODE != "SYSOPER")
                {
                    ...
                    foreach (DataRow dr in (glePin.Properties.DataSource as DataTable).Select("SERIAL='" + glePin.EditValue.ObjectNullString() + "'"))
                    {
                        if (!int.TryParse(dr["ABLEQTY"].ObjectNullString(), out iPinQty))
                        {
                            iPinQty = 0;
                        }
                    }

                    if (glePin.EditValue.ObjectNullString() == "" || iPinQty <= 0)
                    {
                        iDATMessageBox.WARNINGMessage("MSG_ER_PRD_131", this.Text, 3); //JIG (지그)를 선택 해주세요.
                        return;
                    }
                ...
                glePin.Properties.ReadOnly = true;
            ...
            {
                gleUnitNo.Properties.ReadOnly = false;
                glePin.Properties.ReadOnly = false;
            ...
            lcgMount.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
            ...
            switch (btn.Name)
            {
                ...
                case "btnInspection": //검사모드
                    ...
                    lcgMount.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
        ...
        /*통전 검사 실적 저장*/
        private void Set_CircuitInspection(string sData)
        {
            string _strJudge = "Y";
            ...
            if (glePin.EditValue.ObjectNullString() == "")
            {
                iDATMessageBox.ErrorMessage("MSG_ER_PRD_131", this.Text, 3); //JIG (지그)를 선택 해주세요.
                return;
            }
            ...
            if (sData == "FAIL") _strJudge = "N";


            //양품은 바코드 시리얼 채번 후 채번된 시리얼 처리 및 인쇄
            //불량은?????

            // 일단 _strJudge 가 Y 일경우만 
            ...
            WSResults _Result = BASE_db.Execute_Proc("PKGPRD_CURRENT.SET_PROD_PRODUCTION"
                                                    ...
                                                    , glePin.EditValue.ObjectNullString()
                                                    , txtSerial.EditValue.ObjectNullString()
                                                    , _strJudge
        ...
        private void GetGridViewList(string p_sData)
        {
            BASE_DXGridHelper.Bind_Grid(gcList
                                        , "PKGPRD_REPORT.GET_PRODSN_DETAIL_HIST"
                                        , 1
                                        , new string[] { "A_SERIAL" }
                                        , new string[] { p_sData }
                                       );
        ...
        private void GetGridViewList1(string _sSerial)
        {
            BASE_DXGridHelper.Bind_Grid_Int( gcList1
                                           , "PKGPRD_CURRENT.GET_PROD_MOUNT_SERIAL"
                                           , 1
                                           , new string[] { 
                                             "A_CLIENT"
                                           , "A_COMPANY"
        ...
        private void CheckWorkOrder(string workOrder)
        {
            //작업지시 확인
            //작업지시 확인 후 파트넘버를 가져와야 함
            try
            {
                WSResults _Result = BASE_db.Execute_Proc("PKGPRD_PROD.CHK_WORKORDER"
                                                        , 1
                                                        , new string[] {
                                                              "A_CLIENT"
                                                            , "A_COMPANY"
                                                        ...
                                                        , new object[] {
                                                            ...
                                                            , Global.Global_Variable.PLANT
                                                            , workOrder
                                                            , "#0300" } //배판, ESS통전검사는 배판 작업에 연계되므로 배판작업지시를 읽어야 함. 통전은 작업지시 없음
                                                        );


                if (_Result.ResultInt == 0 && _Result.ResultDataSet.Tables[0].Rows.Count > 0)
                {
                    txtSerial.EditValue = null;
                    lblStatus.Text = "";
                }
                ...
                {
                  iDATMessageBox.ErrorMessage("MSG_ER_WO_053", this.Text, 3); //사용가능한 배판 작업지시가 아닙니다.
                }
            }
            catch (Exception)
            {
        ...
        {
            ProcessScanEvent("UNITNO", "P20041");
        }
        ...
        private void txtSerial_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                ProcessScanEvent("PRODSN", txtSerial.EditValue.ObjectNullString());
            }
        }
    }
}
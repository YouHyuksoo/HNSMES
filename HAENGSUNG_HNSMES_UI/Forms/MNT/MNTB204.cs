...
namespace HAENGSUNG_HNSMES_UI.Forms.MNT
{
    ...
    public partial class MNTB204 : HAENGSUNG_HNSMES_UI.Forms.BASE.Form, HAENGSUNG_HNSMES_UI.Class.itfButton
    {
        #region [Form Event]
        ...
        private void MNTB204_Load(object sender, EventArgs e)
        {

        }

        private void MNTB204_Shown(object sender, EventArgs e)
        {
        ...
        private void gvList1_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
        {
            ...
            if (((DevExpress.XtraGrid.GridSummaryItem)e.Item).FieldName.CompareTo("PRODQTY") == 0)
            {
                ...
                if (vExprodQty > 1)
                {
                    decimal dResult = Math.Round(dResultQty / dExprodQty, 2);
            ...
            {
                decimal dExprodQty, dResultQty, dRow;
                ...
                decimal.TryParse(vExprodQty.ObjectNullString(), out dExprodQty);
                decimal.TryParse(vResultQty.ObjectNullString(), out dResultQty);
                decimal.TryParse(vRow.ObjectNullString(), out dRow);

                if (dExprodQty > 1)
                {
                    decimal dResult = Math.Round(dExprodQty / (dResultQty/dRow)  * 1000000, 2);
            ...
            if (((DevExpress.XtraGrid.GridSummaryItem)e.Item).FieldName.CompareTo("ACCRATIO") == 0)
            {
                ...
                decimal dResult = Math.Round(dResultQty, 2);
